...
describe('Backend CKBService - Hash Encoding (Stage 1: Block Timestamp)', () => {
  ...
  describe('buildUnsignedTx', () => {
    ...
    it('should build consistent output across multiple calls', async () => {
      const fileHash = '0x' + 'c'.repeat(64);
      const userAddress = 'ckt1qzda0cr08m85hc8jlnfp3zer7xulejywt49kt2rr0vthywaa50xwsqwgx292hnvmn68xf779vmzrshpmm6epn4c0cgwga';
      
      const result1 = await ckbService.buildUnsignedTx({ fileHash, userAddress });
      const result2 = await ckbService.buildUnsignedTx({ fileHash, userAddress });
      
      // Since we removed timestamp, results should be identical
      expect(result1.outputsData[0]).toBe(result2.outputsData[0]);
      expect(result1.outputsData[0]).toBe(fileHash);
    });
  });
});